...
/**
 * REST controller for managing {@link com.yarmook.realstate.domain.Property}.
 */
@RestController
...
public class PropertyResource {
    ...
    /**
     * {@code POST  /properties/refresh} : Trigger a full refresh of properties from the external feed.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and a summary of the sync.
     */
    @PreAuthorize("hasAuthority('" + AuthoritiesConstants.ADMIN + "')")
    @PostMapping("/refresh")
    public ResponseEntity<PropertyImportService.PropertyRefreshResult> refreshProperties() {
        LOG.debug("REST request to refresh Properties from external source");
        return ResponseEntity.ok(propertyImportService.refreshFromExternal());
    }
