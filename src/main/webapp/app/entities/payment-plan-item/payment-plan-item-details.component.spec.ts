import { vitest } from 'vitest';
import { type MountingOptions, shallowMount } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import PaymentPlanItemDetails from './payment-plan-item-details.vue';
import PaymentPlanItemService from './payment-plan-item.service';
import AlertService from '@/shared/alert/alert.service';

type PaymentPlanItemDetailsComponentType = InstanceType<typeof PaymentPlanItemDetails>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const paymentPlanItemSample = { id: 123 };

describe('Component Tests', () => {
  let alertService: AlertService;

  afterEach(() => {
    vitest.resetAllMocks();
  });

  describe('PaymentPlanItem Management Detail Component', () => {
    let paymentPlanItemServiceStub: SinonStubbedInstance<PaymentPlanItemService>;
    let mountOptions: MountingOptions<PaymentPlanItemDetailsComponentType>['global'];

    beforeEach(() => {
      route = {};
      paymentPlanItemServiceStub = sinon.createStubInstance<PaymentPlanItemService>(PaymentPlanItemService);

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'router-link': true,
        },
        provide: {
          alertService,
          paymentPlanItemService: () => paymentPlanItemServiceStub,
        },
      };
    });

    describe('Navigate to details', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        paymentPlanItemServiceStub.find.resolves(paymentPlanItemSample);
        route = {
          params: {
            paymentPlanItemId: `${123}`,
          },
        };
        const wrapper = shallowMount(PaymentPlanItemDetails, { global: mountOptions });
        const comp = wrapper.vm;
        // WHEN
        await comp.$nextTick();

        // THEN
        expect(comp.paymentPlanItem).toMatchObject(paymentPlanItemSample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        paymentPlanItemServiceStub.find.resolves(paymentPlanItemSample);
        const wrapper = shallowMount(PaymentPlanItemDetails, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
